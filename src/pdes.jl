...
## saving funcitons 
    function pde_save_name(param::Dict{String, Any},t::Float64)
        ...
        return "~/data/pm_pdes_raw/$(name)/[DT,v0,DR,Δx,Lx,ϕa,ϕp]=$([DT,v0,DR,Δx,Lx,ϕa,ϕp])/t=$(s).jld2"
    end

    function speed_save_name(param::Dict{String, Any})
        @unpack DT, v0, DR, Δx, Lx, ϕa, ϕp, name, save_interval = param
        return "~/data/pm_pdes_pro/speeds/$(name)/[DT,v0,DR,Δx,Lx,ϕa,ϕp]=$([DT,v0,DR,Δx,Lx,ϕa,ϕp]).jld2"
    end

    function pde_time_series_save_name(param::Dict{String, Any},t::Float64)
        @unpack DT, v0, DR, Δx, Lx, ϕa, ϕp, name, save_interval = param
        s = round(t ; digits = Int64( -log10(save_interval) ÷ 1 ))
        return "~/data/pm_pdes_pro/$(name)/[DT,v0,DR,Δx,Lx,ϕa,ϕp]=$([DT,v0,DR,Δx,Lx,ϕa,ϕp])/T=$(s)_Δt=$(save_interval).jld2"
    end
    ...
    function pde_vid_save_name(param::Dict{String, Any},t::Float64)
        ...
        s = round(t ; digits = Int64( -log10(save_interval) ÷ 1 ))
        filename = "~/plots/vids/pm_pdes_vids/$(name)/[DT,v0,DR,N,Lx,Ly,ϕa,ϕp]=$([DT,v0,DR,N,Lx,Ly,ϕa,ϕp])/t=$(s).mp4"
        pathname = "~/plots/vids/pm_pdes_vids/$(name)/[DT,v0,DR,N,Lx,Ly,ϕa,ϕp]=$([DT,v0,DR,N,Lx,Ly,ϕa,ϕp])"
