...
using KernelDensity, Peaks, Statistics, ForwardDiff;
...
## ode fns
    ...
    function steady_save_name(param::Dict{String, Any})
        ...
        return "~/data/pm_pdes_pro/steady_state/$(name)/[DT,v0,DR,Δx,Lx,ϕa,ϕp]=$([DT,v0,DR,Δx,Lx,ϕa,ϕp]).jld2"
    end
    ...
    function plot_phase(fig, ax, Pe, font; Lxs = [5,10,20], Δϕ = 0.01)
        ...
        for (i,(ax, Pe, axlim)) in enumerate(zip(axs, Pes, axlims))
            # load binodal
                filename = "~/data/binodal/Pe=$(Pe).jld2"
                data = wload(filename)
    ...
    function show_f(axs,fig,font,param,f; c = "?", point = 1, Δϕ = 0.001)
        ...
        fig_name = "show_f"
            ...
            pathname = "~/plots/pm_stretch/$(fig_name)";
            mkpath(pathname)
            filename = "~/plots/pm_stretch/$(fig_name)/Lx=$(Lx)_Δx=$(Δx)_Pe=$(v0)_ϕa=$(ϕa)_ϕp=$(ϕp).pdf";
...
## outer fns
    ...
    function outer_save_name(param,γ)
        ...
        return "~/data/pm_pdes_pro/outer_sol/$(name)/[DT,v0,DR,Nx,ϕa,γ]=$([DT,v0,DR,Nx,ϕa,d3(γ)]).jld2"
    end
    ...
    function save_waves(wave_map)
        filename = "~/data/pm_pdes_pro/wave_map.jld2"
    ...
    function outer_save_name_2(param)
        ...
        return "~/data/pm_pdes_pro/outer_sol/$(name)/[DT,v0,DR,Nx,ϕa,ϕp]=$([DT,v0,DR,Nx,ϕa,ϕp]).jld2"
    end
...
## outer_4
    ...
    function outer_save_name_4(param,ind)
        @unpack DT, v0, DR, Nx, Lx, name, ϕa, ϕp = param
        return "~/data/pm_pdes_pro/outer_sol/$(name)/[DT,v0,DR,Nx,ϕp,ind]=$([DT,v0,DR,Nx,ϕp,ind]).jld2"
    end


    function outer_save_name_4_old(param,ind)
        @unpack DT, v0, DR, Nx, Lx, name, ϕa, ϕp = param
        return "~/data/pm_pdes_pro/outer_sol/$(name)/[DT,v0,DR,Nx,ϕa,ind]=$([DT,v0,DR,Nx,ϕa,ind]).jld2"
    end
...
## bonus full sol fns
    ...
    function save_name_full(param)
        @unpack DT, v0, DR, Nx, Lx, name, ϕa, ϕp = param
        return "~/data/pm_pdes_pro/full_sol/$(name)/[DT,v0,DR,Lx,Nx,ϕa,ϕp]=$([DT,v0,DR,Lx,Nx,ϕa,ϕp]).jld2"
    end
...
## outer 5 
    ...
    function outer_save_name_5(param,ϕ,γ)
        @unpack DT, v0, DR, Nx, Lx, name, ϕa, ϕp = param
        return "~/data/pm_pdes_pro/outer_sol/$(name)/[DT,v0,DR,Nx,ϕ,γ]=$([DT,v0,DR,Nx,ϕ,γ]).jld2"
    end
...
## outer 6 
    ...
    function outer_save_name_6(param,ind,γ)
        @unpack DT, v0, DR, Nx, Lx, name, ϕa, ϕp = param
        return "~/data/pm_pdes_pro/outer_sol/$(name)/[DT,v0,DR,Nx,ind,γ]=$([DT,v0,DR,Nx,ind,γ]).jld2"
    end
